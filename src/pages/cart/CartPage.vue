  ...
  <section class="container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col lg:flex-row gap-8">
      ...
      <div class="flex-1 bg-gray-50 p-6 rounded-lg shadow-sm">
        <h2 class="text-xl font-bold mb-4">Shopping Bag</h2>
        <p class="text-sm text-gray-600 mb-6">{{ cartItems.length }} items in your bag</p>

        <div v-for="(item, index) in cartItems" :key="index" class="flex items-center justify-between mb-4 border-b pb-4">
          <!-- Item Details -->
          <div class="flex items-center ">
            <img :src="item.image" alt="Product Image" class="w-20 h-20 rounded-lg mr-4" />
            <div>
          ...
          <div class="flex items-center gap-4">
            <select v-model="item.quantity" class="border rounded-md px-2 py-1 text-gray-700">
              <option v-for="n in 10" :key="n" :value="n">{{ n }}</option>
            </select>
            <div class="text-base font-bold text-gray-800">${{ (item.price * item.quantity).toFixed(2) }}</div>
            <button
              @click="removeItem(item)"
              class="text-red-500 text-sm  focus:outline-none rounded-full hover:bg-red-50  transition duration-150 ease-in-out p-2"
            >
      ...
      <div class="w-full lg:w-1/3 bg-white p-6 rounded-lg shadow-sm">
        ...
        <div class="mb-6">
          ...
          <div class="flex gap-2">
            ...
            />
            <button class="bg-blue-600 text-white rounded-md px-4 py-2 hover:bg-blue-700">
              Apply
            </button>
          </div>
        </div>
        ...
        <button class="mt-4 w-full bg-red-600 text-white rounded-md px-4 py-2 hover:bg-red-700">
          Proceed to Checkout
        </button>
      </div>
    </div>
